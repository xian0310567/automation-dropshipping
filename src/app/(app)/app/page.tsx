      ...
      <section className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-5 py-6 md:flex-row md:items-center md:justify-between lg:px-8">
          ...
          <div className="flex flex-wrap gap-2">
            <ActionButton icon={<RotateCcw size={18} />} label="주문 동기화" />
            <ActionButton icon={<Send size={18} />} label="CS 답변 검토" primary />
          </div>
        </div>
      </section>
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-lg border border-zinc-200 bg-white">
            ...
            <div className="divide-y divide-zinc-100">
              ...
              <ChecklistItem done label="tenant schema contracts" />
              <ChecklistItem label="Coupang HMAC smoke test" />
              <ChecklistItem label="Ownerclan fulfillment readiness" />
            </div>
          </section>
        </aside>
      </div>
...
const approvalRows = [
  {
    ...
    title: "오너클랜 발주 필요",
    detail: "신규 결제완료 주문 중 공급사 발주가 필요한 건",
    count: 9,
    status: "승인 대기",
  },
  {
